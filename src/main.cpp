...
void setup() {
  ...
  init_rs_cfg();
#if 1
  LOCAL::NetworkManager::instance().config_dhcpcd("/etc/dhcpcd.conf");
  LOCAL::NetworkManager::instance().config_wpa_supplicant("/etc/wpa_supplicant.conf");
  LOCAL::NetworkManager::instance().begin();
...
static void check_network_state() {
  auto &network_manager = LOCAL::NetworkManager::instance();
...
static void send_msg(const String &str, const char *server_ip,
  ...
  auto &udp = LOCAL::NetworkManager::instance();
